	...
	for (t = 0; t < num_tchunks; t++)
	{
		...
		for (dm_range = 0; dm_range < range; dm_range++)
		{
			...
			if (enable_analysis == 1) {
				analysis_GPU(dm_range, tstart_local, t_processed[dm_range][t], ( t_processed[dm_range][t] + maxshift ), nchans, maxshift, max_ndms, ndms, outBin, sigma_cutoff, sigma_constant, d_output, dm_low, dm_high, dm_step, tsamp);
